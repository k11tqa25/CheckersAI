...
def timeit(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        global time_spent
        time_spent = round(time.perf_counter() - t1, 2)
        print(func.__name__, " executes", time_spent, 's')
        return result
    return wrapper
    ...
    if len(player.jump_available_tokens) != 0:
        for jt in player.jump_available_tokens:
            for path in jt.jump_path:
                ...
                p.jump(tkn, path)
                save_boards(p.board, f"- [D:{depth}] {jt.color} Jump {jt.position} to {path[-1]}, path = {path}:")
        ...
        for mt in player.move_available_tokens:
            for move in mt.available_moves:
                ...
                save_boards(p.board, f"- [D:{depth}] {mt.color} Move {mt.position} to {mt.get_landing(move)}")
                v_next, next_moves = apply_alg(p.opponent, not is_max, depth + 1, a, b)
                del p
                if is_max:
                    if v < v_next:
...
class Game:
    ...
    def play(self):
        ...
        """
        save_boards(self.board, "original board:")
        v, moves = apply_alg(self.player)

        return v, moves

    def display_result(self, v, moves):
        print_board(self.board, "original board:")
        display_player = copy.deepcopy(self.player)
        i = 1
        while True:
            m = moves[i]
            ...
            for token in p.tokens:
                if token.readable_position == m[1]:
                    ...
                    print_board(p.board)
                    print(f"h = {display_player.get_heuristic()}")
                    break
            if i + 1 in moves:
                moves = moves[i+1]
                i += 1
...
class Player:
    ...
    def get_heuristic(self):
        def heuristic(player):
            ...
            return piece_count + king + jump_reward + position_reward + number_of_moves + protection
        bonus = 0
